...
namespace  Enemy00
{
	...
	void Object::Received(BChara* from_, BChara::AttackInfo at)
	{
		...
		//吹き飛ばす
		auto shot = ge->GetTask_Group_G<BChara>("Shot");
		for (auto id = (*shot).begin(); id != (*shot).end(); ++id)
		{
			if ((*id)->angle_LR == Left)
			{
				moveVec = ML::Vec2(-4, -9);
	...
	void  Object::Think()
	{
		...
		case  Stand:	//立っている
			...
			break;
		case  Walk:		//歩いている
			if (CheckFrout_LR()) { nm = Turn; }		//方向転換
			if (!CheckFoot()) { nm = Fall; }		//落下判定
			break;
		case  Jump:		//上昇中
